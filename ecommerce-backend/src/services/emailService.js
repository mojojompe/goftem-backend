...
const sendOtpEmail = (to, otp) => {
  const mailOptions = {
    ...
    html: fs
      .readFileSync(path.join(__dirname, "../templates/otpEmail.html"), "utf8")
      .replace("{{otp}}", otp),
  };
...
const sendPaymentConfirmationEmail = (to, orderDetails) => {
  const mailOptions = {
    ...
    html: fs
      .readFileSync(
        path.join(__dirname, "../templates/paymentConfirmationEmail.html"),
        "utf8"
      )
...
const sendCheckoutEmail = (to, orderSummary) => {
  const mailOptions = {
    ...
    html: fs
      .readFileSync(
        path.join(__dirname, "../templates/checkoutEmail.html"),
        "utf8"
      )
...
const notifyNewStockEmail = (to, productName) => {
  const mailOptions = {
    ...
    html: fs
      .readFileSync(
        path.join(__dirname, "../templates/newStockEmail.html"),
        "utf8"
      )
